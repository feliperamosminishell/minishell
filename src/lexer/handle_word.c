...
/*   Updated: 2026/04/16 14:46:03 by juan-her         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			char **value)
{
	...
	else if (line[lx->i] == '$' && !lx->in_s && !lx->after_heredoc)
		ft_globlal(line, lx, value);
	else
		ft_word(line, lx, value);
}

static int	ft_q(t_lexer *lx)
{
	if ((lx->in_s || lx->in_d))
		return (1);
	return (0);
...
static int	ft_is(const char *line, t_lexer *lx)
{
	if (!ft_is_operator(line[lx->i]) && !ft_isspace(line[lx->i]))
		return (1);
	return (0);
}
